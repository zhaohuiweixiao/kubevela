...
var _ = Describe("Addon Test", func() {
	...
	Context("Enable dependency addon test", func() {
		It("mock-dependence addon is not enable and enable mock-dependence-rely without specified clusters", func() {
			...
			Eventually(func(g Gomega) {
				...
				Expect(topologyPolicyValue["clusterLabelSelector"]).Should(Equal(map[string]string{}))
		...
		It("mock-dependence addon enabled with specified clusters and enable mock-dependence-rely without specified clusters", func() {
			...
			Eventually(func(g Gomega) {
				...
				Expect(topologyPolicyValue["clusterLabelSelector"]).Should(Equal(BeNil()))
			}, 600*time.Second).Should(Succeed())
			// 2. enable mock-dependence-rely addon without clusters
			output1, err := e2e.Exec("vela addon enable mock-dependence-rely")
			Expect(err).NotTo(HaveOccurred())
			...
			Eventually(func(g Gomega) {
				...
				Expect(topologyPolicyValue["clusterLabelSelector"]).Should(Equal(map[string]string{}))
				Expect(topologyPolicyValue["clusters"]).Should(Equal(BeNil()))
			}, 30*time.Second).Should(Succeed())
